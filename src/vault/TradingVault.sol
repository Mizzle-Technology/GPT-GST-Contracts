    ...
    ITradingVault
{
     ...
     */
    function queueWithdrawal(address token, uint256 amount) external onlyRole(ADMIN_ROLE) whenNotPaused nonReentrant {
        ...
        emit WithdrawalQueued(requestId, token, amount, safeWallet, block.timestamp, block.timestamp + WITHDRAWAL_DELAY);
    }
     ...
     */
    function executeWithdrawal(bytes32 requestId) external onlyRole(ADMIN_ROLE) whenNotPaused nonReentrant {
        ...
        require(ERC20Upgradeable(request.token).balanceOf(address(this)) >= request.amount, "Insufficient balance");

        ERC20Upgradeable(request.token).safeTransfer(request.transfer_to, request.amount);
        request.executed = true;

        emit WithdrawalExecuted(requestId, request.token, request.amount, request.transfer_to, block.timestamp);
    }
     ...
     */
    function cancelWithdrawal(bytes32 requestId) external onlyRole(ADMIN_ROLE) whenNotPaused nonReentrant {
        WithdrawalRequest storage request = withdrawalRequests[requestId];
        require(request.amount > 0, "Invalid request ID");
        require(!request.executed, "Already executed");
        ...
        emit WithdrawalCancelled(requestId, request.token, request.amount, request.transfer_to, block.timestamp);
    }
     ...
     */
    function withdraw(address token, uint256 amount) external onlyRole(ADMIN_ROLE) whenNotPaused nonReentrant {
        require(amount > 0, "Amount must be greater than 0");
        require(token != address(0), "Invalid token address");
     ...
     */
    function setWithdrawalThreshold(uint256 _threshold) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
